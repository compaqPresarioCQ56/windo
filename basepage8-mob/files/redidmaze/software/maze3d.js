...
function maze3dMain(mode){my.version='0.61';my.typ=typeof mode!=='undefined'?mode:'bla';w=Math.min(5000,window.innerWidth-0)
h=Math.min(9999,window.innerHeight-0)
var s='';s+='';s+='';s+='<canvas id="canvas1" style="position: fixed; border: none; left: -1; right:-1; top: -1; bottom: -1;"></canvas>';s+='';s+='';s+='<button id="options" style="position: absolute; right: 5px; top:5px; font-size: 15px; z-index:3; font: 16px Arial; color: #000aae; " class="togglebtn"  onclick="popShow()" >Options</button>';s+=popHTML()
s+='<div id="copyrt" style="font: 10px Arial; color: #6600cc; position:absolute; bottom:-15px; left:5px; text-align:center;">'+my.version+'</div>';s+='';document.write(s);el=document.getElementById('canvas1');var ratio=6;el.width=w*ratio;el.height=h*ratio;el.style.width=w+"px";el.style.height=h+"px";g=el.getContext("2d");g.setTransform(ratio,0,0,ratio,0,0);my.clrs=[["Blue",'#0000ff'],["Red",'#FF0000'],["Black",'#000000'],["Green",'#00cc00'],["Orange",'#FFA500'],["Slate Blue",'#6A5ACD'],["Lime",'#00FF00'],["Spring Green",'#00FF7F'],["Teal",'#008080'],["Gold",'#ffd700'],["Med Purple",'#aa00aa'],["Light Blue",'#ADD8E6'],["Navy",'#000080'],["Purple",'#800080'],["Dark SeaGreen",'#8FBC8F'],["White",'#000000'],["Yellow",'#ffff00'],["Light Yellow",'#ffffdd']];console.log("my.typ",my.typ);my.cellMax=100
...
update()},false);}
function Player(x,y,direction){this.x=x;this.y=y;this.direction=direction;this.weapon=new Img('wood6.png',100,32);this.paces=0;}
...
console.log('success')
success()}}};Player.prototype.update=function(controls,map,seconds){if(controls.left)this.rotate(-Math.PI*seconds*0.7);if(controls.right)this.rotate(Math.PI*seconds*0.7);if(controls.forward)this.walk(3*seconds,map);if(controls.backward)this.walk(-3*seconds,map);if(controls.left||controls.right||controls.forward||controls.backward){return true}else{return false}};function Map(){this.wallGrid=[]
this.skybox=new Img('bg.png',1000,750);this.wallTexture=new Img('wood6.png',1000,1000);this.light=0;}
...
this.resolution=resolution;this.spacing=this.width/resolution;this.focalLength=focalLength||1;this.range=MOBILE?16:9;this.lightRange=5;this.scale=(this.width+this.height)/10;}
...
if(ambient>0){this.ctx.fillStyle='#000000';this.ctx.globalAlpha=ambient*0.1;this.ctx.fillRect(0,this.height*0.5,this.width,this.height*0.5);}
...
if(cell==undefined){ctx.fillStyle='#00000099';}else{ctx.fillStyle=cell[6];}
ctx.fillRect(left,wall.top,width,wall.height);ctx.fillStyle='#00000099';ctx.globalAlpha=Math.max((step.distance+step.shading)/this.lightRange-map.light,0);ctx.fillRect(left,wall.top,width,wall.height);break
...
function popHTML(){var s='';s+='<div id="optpop" style="position:absolute; left:-450px; top:20px; width:350px; height:160px; padding: 5px; border-radius: 9px; background-color: #af0a0a99; backdrop-filter: blur(10px); box-shadow: 10px 10px 5px 0px rgba(40,40,40,0.7); transition: all linear 0.3s; opacity:0; text-align: center; ">';s+='<div id="info">&nbsp;</div>'
